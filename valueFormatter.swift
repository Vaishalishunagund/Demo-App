...
public class StringValueFormatterForGraphData: NSObject, IAxisValueFormatter {
    weak var chart: BarLineChartViewBase?
    var XdataSet: [String]?
    
    init(chart: BarLineChartViewBase) {
        self.chart = chart
        XdataSet = theAppModel.sharedInstance.getXaxisDataSet()
    }
    
    init(lineChart: ChartViewBase) {
        self.chart = lineChart as? BarLineChartViewBase
        XdataSet = theAppModel.sharedInstance.getXaxisDataSet()
    }
    ...
    public func stringForValue(_ value: Double, axis: AxisBase?) -> String {
        ...
        if let chart = chart,
            chart.visibleXRange > 30 * 6 {
            if XdataSet!.count > 0{
                itemToReturn = XdataSet!.getItem(index: Int(value))
            }
        }else{
            if XdataSet!.count > 0{
                itemToReturn = XdataSet!.getItem(index: Int(value))
